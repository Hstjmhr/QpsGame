...
func (r Resolver) Build(target resolver.Target, cc resolver.ClientConn, opts resolver.BuildOptions) (resolver.Resolver, error) {
	...
	if err != nil {
		logs.Fatal("client connect etcd err:%v", err)
	}
	...
	for _, ev := range events {
		...
		case clientv3.EventTypePut:
			...
			addr := resolver.Address{
				Addr:       server.Addr,
				Attributes: attributes.New("wight", server.Weight),
			}
			if !Exist(r.srvAddrList, addr) {
				...
				if err != nil {
					logs.Error("grpc client UpdateState(EventTypePut) UpdateState failed,name=%s,err:%v", r.key, err)
				}
			}
		case clientv3.EventTypeDelete:
			// 接收到delete操作 删除r.srcAddrList其中匹配的
